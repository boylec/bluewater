...
This tutorial walks through one full OOD (Officer of the Deck — the
agent on call right now) watch on a Bluewater ship. It's how you
build the mental model — once you've watched a watch, the rest of
the pack starts making sense.

> **Background:** [`docs/concepts.mdx`](./concepts.mdx) explains the
> watch-standing metaphor in software-engineering terms;
> [`docs/glossary.mdx`](./glossary.mdx) is the term lookup. Skim
> either if a word here surprises you.
...
gc events --type bluewater.casualty --since 1h
```
...
gc event emit bluewater.casualty \
  --payload '{"kind":"flooding","severity":"high"}'
```
